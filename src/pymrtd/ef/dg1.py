...
class DataGroup1(asn1.OctetString):
    class_ = 1
    tag = 31
    _parsed = None
    _type = None
    ...
    def load(cls, encoded_data: bytes, strict=False, **kwargs):
        v: DataGroup1 = super().load(encoded_data, strict, **kwargs)
        clen = len(v.contents)
    ...
    def _parse_td1(self):
        ...
        self._parsed["document_number_cd"] = self._read_with_filter(
            14, 1
        )  # document number check digit, could be char '<'
        ...
        self._parsed["date_of_expiry_cd"] = self._read_cd(
            44
        )  # document doe check digit
    ...
    def _parse_td3(self):
        ...
        self._parsed["document_number_cd"] = self._read_cd(
            53
        )  # document number check digit
    ...
    def _parseExtendedDocumentNumber(self):
        ...
        if (
            self._parsed["document_number_cd"] == "<"
            and len(self._parsed[fn_opt_data]) > 0
        ):
    ...
    def _read_cd(self, idx) -> int:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid check digit character '{scd}' in MRZ at position {idx}"
            )
    ...
    def _read_date_of_birth(self, idx, len):
        ...
        if (
            date is not None and date > datetime.today().date()
        ):  # reduce date for 100 years if greater then current date
...
class DG1(DataGroup):
    tag = 1
    _content_spec = DataGroup1

    @property
    def mrz(self) -> DataGroup1:
        return self.content
