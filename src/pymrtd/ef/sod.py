...
class SODSignedData(cms.MrtdSignedData):
    ...
    cms.cms_register_encap_content_info_type(
        "ldsSecurityObject", oids.id_mrtd_ldsSecurityObject, LDSSecurityObject
    )
    cms.cms_register_encap_content_info_type("data", oids.id_data, _DataChoice)
...
class SOD(ElementaryFile):
    ...
    def load(cls, encoded_data: bytes, strict=False) -> "SOD":
        ...
        try:
            ...
            if ctype != "signed_data":  # ICAO 9303-10-p21
                raise SODError(
                    f"Invalid content type: '{ctype}', expected 'signed_data'"
                )
            ...
            if not cls._valid_content_type(
                s.signedData.contentType, strict
            ):  # s.signedData.contentType.dotted != oids.id_mrtd_ldsSecurityObject:
                raise SODError(
                    f"Invalid encapContentInfo type: {s.signedData.contentType.dotted}, expected {oids.id_mrtd_ldsSecurityObject}"
                )

            if not (0 <= s.ldsSecurityObject.version.value <= 1):
                raise SODError(
                    f"Unsupported LDSSecurityObject version: {s.ldsSecurityObject.version.value}, expected 0 or 1"
                )

            assert (
                isinstance(s.signedData.certificates[0], x509.DocumentSignerCertificate)
                if len(s.signedData.certificates)
                else True
            )
    ...
    def _valid_content_type(cls, ct, strict):
        ...
        return oid == oids.id_mrtd_ldsSecurityObject or (
            strict is False and oid in cls._allowedSodContentTypes
        )
    ...
    def __str__(self):
        ...
        if self._str_rep is None:
            self._str_rep = super().__str__().replace("EF(", "EF.SOD(", 1)
    ...
    def getDscCertificate(
        self, si: cms.SignerInfo
    ) -> Optional[x509.DocumentSignerCertificate]:
        """
